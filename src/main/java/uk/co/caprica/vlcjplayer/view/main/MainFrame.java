...
public final class MainFrame extends BaseFrame {
    ...
    //---------------------NUEVO----------------------
    private final Action partyConnectAction;
    private final Action partyDisconnectAction;
    private final Action partySendAction;
    private final Action partyStartAction;
    private final Action partyEndAction;
    //--------------------/NUEVO----------------------
...
//                    mediaPlayerComponent.getMediaPlayer().subpictures().setSubTitleFile(file);
        ...
        //---------------------NUEVO----------------------
        partyConnectAction = new StandardAction(resource("menu.party.item.connect")) {
            @Override
            public void actionPerformed(ActionEvent e) {
            	String ip = JOptionPane.showInputDialog("Type the IP address you want to connect to");
            	String port = JOptionPane.showInputDialog("Type the port used in that machine for the party");
            	partyClient = new Client(ip, Integer.parseInt(port));
            	application().getMediaPlayerActions().getPlayBackPlayAction().setPartyClient(partyClient);
            }
        };
        
        partyDisconnectAction = new StandardAction(resource("menu.party.item.disconnect")) {
            @Override
            public void actionPerformed(ActionEvent e) {
            	partyClient.setPartner("");
            	partyClient.setPort(0);
            	application().getMediaPlayerActions().getPlayBackPlayAction().setPartyClient(new Client("",0));
            }
        };
        
        partyStartAction = new StandardAction(resource("menu.party.item.start")) {
            @Override
            public void actionPerformed(ActionEvent e) {
            	String port = JOptionPane.showInputDialog("Type the port you want to use for your server");
                partyServer = new Server(Integer.parseInt(port));
                partyServer.start();
            }
        };
        
        partyEndAction = new StandardAction(resource("menu.party.item.end")) {
            @Override
            public void actionPerformed(ActionEvent e) {
            	try {
					partyServer.getServerSocket().close();
        ...
        partySendAction = new StandardAction(resource("menu.party.item.send")) {
            @Override
            public void actionPerformed(ActionEvent e) {
            	if (JFileChooser.APPROVE_OPTION == fileChooser.showOpenDialog(MainFrame.this)) {
                    File file = fileChooser.getSelectedFile();
                    String mrl = file.getAbsolutePath();
                    partyClient.send(mrl,file.getName());
                }
            }
        };
        //---------------------/NUEVO----------------------
...
//        videoMenu.add(videoOutputMenu);
        ...
        partyMenu = new JMenu(resource("menu.party").name());
        partyMenu.setMnemonic(resource("menu.party").mnemonic());
        partyMenu.add(new JMenuItem(partyStartAction));
        partyMenu.add(new JMenuItem(partyEndAction));
        partyMenu.add(new JMenuItem(partyConnectAction));
        partyMenu.add(new JMenuItem(partyDisconnectAction));
        partyMenu.add(new JMenuItem(partySendAction));
            ...
            public void timeChanged(MediaPlayer mediaPlayer, long newTime) {
                SwingUtilities.invokeLater(new Runnable() {
                    ...
                    public void run() {
                        ...
                        //---------------------NUEVO----------------------
                        if(partyClient != null && (Math.abs(newTime - lastTimeNotified) > 3000)) {
                        	//Aqui establezco que tiene que haber más de 3 segundos entre el tiempo nuevo y el anterior notificado
                        	//para que se envíe la informacion. Esto es debido a que esta funcion se ejecuta cada "tick" y estaria
                        	//enviando informacion muchas veces por segundo. Con esto lo soluciono y no da problemas.
                        	partyClient.time(newTime);
                        }
                        lastTimeNotified = newTime;
                        //---------------------/NUEVO----------------------
